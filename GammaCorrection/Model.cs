...
using Assimp;
using System.Linq;
using static GammaCorrection.Mesh;
...
namespace GammaCorrection
{
    class Model
    {
        ...
        private Mesh processMesh(Assimp.Mesh mesh, Scene scene)
        {
            ...
            List<GammaCorrection.Mesh.Texture> textures = new List<GammaCorrection.Mesh.Texture>();
            ...
            if (mesh.MaterialIndex > 0)
            {
                ...
                List<GammaCorrection.Mesh.Texture> diffuseMaps = loadMatericalTextures(material, TextureType.Diffuse, "diffuse");
                for (int i = 0; i < diffuseMaps.Count; i++)
                {
                    textures.Insert(textures.Count, diffuseMaps[i]);
                }
                List<GammaCorrection.Mesh.Texture> specularMaps = loadMatericalTextures(material, TextureType.Specular, "specular");
        ...
        /// <returns></returns>
        private List<GammaCorrection.Mesh.Texture> loadMatericalTextures(Material material, TextureType type, string typename)
        {
