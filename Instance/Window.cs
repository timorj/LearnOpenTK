...
namespace Instance
{
    class Window : GameWindow
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            quadShader = new Shader("../../Shaders/instance.vert", "../../Shaders/instance.frag");
            quadShader.Use();

            GL.GenBuffers(1, out quadVBO);
            GL.BindBuffer(BufferTarget.ArrayBuffer, quadVBO);
            GL.BufferData(BufferTarget.ArrayBuffer, instanceQuadVertices.Length * sizeof(float), instanceQuadVertices, BufferUsageHint.StaticDraw);

            GL.GenVertexArrays(1, out quadVAO);
            GL.BindVertexArray(quadVAO);

            GL.BindBuffer(BufferTarget.ArrayBuffer, quadVBO);
            var positionLocaion = quadShader.GetAttribLocation("aPos");
            GL.EnableVertexAttribArray(positionLocaion);
            GL.VertexAttribPointer(positionLocaion, 2, VertexAttribPointerType.Float, false, 5 * sizeof(float), 0);

            GL.BindBuffer(BufferTarget.ArrayBuffer, quadVBO);
            var colorLocation = quadShader.GetAttribLocation("aColor");
            ...
            GL.BufferData(BufferTarget.ArrayBuffer, translations.Length * Vector2.SizeInBytes,translations , BufferUsageHint.StaticDraw);
            GL.BindBuffer(BufferTarget.ArrayBuffer, 0);

            GL.BindBuffer(BufferTarget.ArrayBuffer, instanceVBO);
            var offsetlocation = quadShader.GetAttribLocation("offset");
            GL.EnableVertexAttribArray(offsetlocation);
            GL.VertexAttribPointer(offsetlocation, 2, VertexAttribPointerType.Float, false, Vector2.SizeInBytes, 0);
