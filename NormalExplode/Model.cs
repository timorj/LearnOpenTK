...
using Assimp;
using System.Linq;
using static NormalExplode.Mesh;
...
namespace NormalExplode
{
    class Model
    {
        ...
        private Mesh processMesh(Assimp.Mesh mesh, Scene scene)
        {
            ...
            List<NormalExplode.Mesh.Texture> textures = new List<NormalExplode.Mesh.Texture>();
            ...
            if (mesh.MaterialIndex > 0)
            {
                ...
                List<NormalExplode.Mesh.Texture> diffuseMaps = loadMatericalTextures(material, TextureType.Diffuse, "diffuse");
                for (int i = 0; i < diffuseMaps.Count; i++)
                {
                    textures.Insert(textures.Count, diffuseMaps[i]);
                }
                List<NormalExplode.Mesh.Texture> specularMaps = loadMatericalTextures(material, TextureType.Specular, "specular");
        ...
        /// <returns></returns>
        private List<NormalExplode.Mesh.Texture> loadMatericalTextures(Material material, TextureType type, string typename)
        {
